...
export async function priceCart(
  ...
  const event = await first(db.select().from(events).where(eq(events.id, eventId)));
  if (!event) throw new Error("Event not found.");
  const q = await quoteCart({ event, lines, code });
  return {
...
export async function liveAvailability(eventId: string, showDateId: string | null) {
  const avail = await zoneAvailability(eventId, showDateId);
  return {
    zones: Object.fromEntries([...avail].map(([k, v]) => [k, v.available])),
    takenSeatIds: [...(await unavailableSeatIds(eventId, showDateId))],
  };
}
...
export async function startCheckout(input: {
  ...
  const event = await first(db.select().from(events).where(eq(events.id, input.eventId)));
  ...
  try {
    const { orderId, publicId, quote } = await createPendingOrder({
      event,
      cartId: id(),
      lines: input.lines,
...
export async function completePayment(
  ...
  const order = await first(db.select().from(orders).where(eq(orders.publicId, publicId)));
  if (!order) return { ok: false as const, error: "Order not found." };
  if (order.status === "paid") return { ok: true as const };

  if (!verifyCallback(payload)) {
    await failOrder(order.id, "failed");
